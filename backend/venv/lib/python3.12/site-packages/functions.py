...
from collections.abc import Iterable, Sequence  # Update imports for Python 3
...
def walk(inner, outer, data):
    ...
    def process_node(inner, k, v):
        if not isinstance(v, Iterable) or isinstance(v, str):  # Replace basestring with str
            return inner(k, v)
    ...
    nodes = tuple(map(lambda kv: process_node(inner, kv[0], kv[1]), data.items()))  # Fix lambda unpacking
...
def walk_replace(smap, data):
    ...
    def process_node(k, v):
        ...
        if isinstance(v, Sequence):
            rows = ()
            for row in v:
                if isinstance(row, str):  # Replace basestring with str
                    rows += (row,)
                else:
    ...
    try:
        nodes = tuple(map(lambda kv: process_node(kv[0], kv[1]), data.items()))  # Fix lambda unpacking
...
def dict_invert(dict):
    return {v: k for (k, v) in dict.items()}  # Fix iteritems to items
...
def dissoc(fdict, key, *ks):
    keys = (key,) + ks
    return {k: v for k, v in fdict.items() if k not in keys}  # Fix iteritems to items
